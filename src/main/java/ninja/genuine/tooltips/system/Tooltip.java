...
public class Tooltip {
    ...
    public Tooltip(EntityPlayer player, TooltipTarget target) {
        this.target = target;
        syncSettings();
        generateTooltip(player, target.getItem());
    }
    ...
    public EnumChatFormatting getRarityColor() {
        return this.target.getItem().getRarity().rarityColor;
    }
    ...
    private void generateTooltip(EntityPlayer player, ItemStack item) {
        if (useNei)
            try {
                text = (List<String>) info.invoke(null, this.target.getItem(), null, Minecraft.getMinecraft().gameSettings.advancedItemTooltips);
            } catch (final Exception e) {
            }
